function powerGeneratedwithoutTune = PowerwithoutTuning(filepath)
%SOUND PRESSURE LEVELS
source = dsp.AudioFileReader(filepath);
...
volt_Factor = ampAtResonant/max1;
...
VoltageGen = (-3.418 * 10^-3 * volt_Factor).*maxParray ;
Z_load= 200 ; %resistance in ohms
Total_Time= t(end)-t(1);
Power= (1/Total_Time)*sum((VoltageGen.^2))/Z_load;
powerValues = (VoltageGen.^2) / Z_load;
tdel = 2.2676e-05;
totalEnergy = sum(powerValues * tdel); % Total energy in Joules
totalTime = length(VoltageGen) * tdel; % Total time
averagePowerOutput = totalEnergy / totalTime; % Average power in Watts
powerGeneratedwithoutTune = averagePowerOutput;
